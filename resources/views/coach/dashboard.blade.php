    ...
    <div class="container mx-auto flex justify-between items-center">
        ...
        <div class="flex items-center gap-3">
            <a href="{{ route('coach.reports.index') }}"
               class="bg-emerald-500 hover:bg-emerald-600 text-white px-3.5 py-1.5 rounded-lg text-sm font-medium transition flex items-center gap-1.5 shadow-sm">
            ...
            <form action="{{ route('logout') }}" method="POST">
                @csrf
                <button type="submit"
                    class="bg-red-500 hover:bg-red-600 px-3 py-1.5 rounded-lg text-sm transition">
                    Keluar
                </button>
            </form>
        </div>
...
<main class="container mx-auto p-4 md:p-8 space-y-12">
                ...
                <div class="lg:col-span-1">
                    <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
                        ...
                        <div class="space-y-4">
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-600 mb-1 font-bold">
                                    Evaluasi / Catatan Pelatih
                                </label>
                                <textarea name="evaluation" rows="4"
                                    class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-green-500 focus:border-green-500 p-2.5 border bg-green-50/30"
                                    placeholder="Berikan evaluasi hasil latihan hari ini..."></textarea>
                                <p class="text-[10px] text-gray-400 mt-1">
                                    *Catatan harian ini akan muncul di dashboard atlet/orang tua.
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden h-full flex flex-col">
                        ...
                        <div class="p-6 border-b border-gray-200 bg-gray-50 flex flex-col md:flex-row md:items-center justify-between gap-4">
                            ...
                            <div class="relative">
                                ...
                                <input type="text" id="searchAthlete"
                                    placeholder="Cari nama atlet..."
                                    class="pl-10 pr-4 py-2 border rounded-lg focus:ring-blue-500 focus:border-blue-500 w-full md:w-64">
                            </div>
                        </div>

                        <div class="overflow-x-auto max-h-[400px] overflow-y-auto flex-grow">
                            <table class="w-full text-left border-collapse">
                                <thead class="bg-gray-100 text-gray-600 uppercase text-xs font-bold sticky top-0 z-10">
                                    <tr>
                                            ...
                                            <tr class="hover:bg-blue-50 transition cursor-pointer"
                                                ...
                                                <td class="px-6 py-4 text-center">
                                                    <input type="checkbox"
                                                        name="athletes[]"
                                                        value="{{ $athlete->id }}"
                                                        class="athlete-checkbox w-5 h-5 text-blue-600 border-gray-300 rounded focus:ring-blue-500"
                                                        onclick="event.stopPropagation(); updateCounter();">
                                                </td>
                        ...
                        <div class="p-6 bg-gray-50 border-t flex flex-col md:flex-row justify-between items-center gap-4">
                            ...
                            <button type="submit"
                                class="w-full md:w-auto bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-10 rounded-lg shadow-lg transition-all transform hover:scale-[1.02]">
                                Simpan Presensi
                            </button>
                        </div>
    ...
    {{-- DATA RIWAYAT --}}
    @php
        $historyList = collect($attendanceHistory ?? []);
        $groupedHistory = $historyList->groupBy(function ($item) {
    ...
    <section class="mt-12 space-y-6">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <div class="flex items-center bg-white border border-blue-200 rounded-lg px-3 py-2 shadow-sm focus-within:ring-2 focus-within:ring-blue-500 min-w-[220px]">
                ...
                <select id="historyMonthFilter"
                    onchange="filterHistoryMonth(this.value)"
                    class="w-full bg-transparent border-none focus:ring-0 text-gray-700 font-medium cursor-pointer">

                    <option value="all">Semua Riwayat</option>

                    @foreach($groupedHistory as $monthYear => $data)
                        <option value="{{ \Illuminate\Support\Str::slug($monthYear) }}">
                            {{ $monthYear }}
                        </option>
                    @endforeach
                </select>
        ...
        <div id="historyContainer">
            ...
            @if($groupedHistory->isNotEmpty())
                ...
                @foreach($groupedHistory as $monthYear => $items)

                    <div id="history-{{ \Illuminate\Support\Str::slug($monthYear) }}"
                        class="month-section bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden mb-8">

                        <div class="p-4 bg-gray-50 border-b flex justify-between items-center">
                            <span class="font-bold text-blue-700 uppercase text-sm tracking-widest">
                                {{ $monthYear }}
                            </span>

                            <span class="text-xs bg-blue-100 text-blue-600 px-2 py-1 rounded font-bold">
                                {{ count($items) }} Sesi
                            </span>
                        </div>
                                        ...
                                        <tr class="hover:bg-gray-50 transition">
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <div class="flex flex-col">
                                                    <span class="font-bold text-gray-700">
                                                        {{ \Carbon\Carbon::parse($history->tanggal)->translatedFormat('d') }}
                                                    </span>
                                                    <span class="text-[10px] text-gray-400 uppercase">
                                                        {{ \Carbon\Carbon::parse($history->tanggal)->translatedFormat('M Y') }}
                                                    </span>
                                                </div>
                                            </td>
                                            ...
                                            <td class="px-6 py-4">
                                                @if($history->evaluation)
                                                    <span class="text-sm text-gray-500 line-clamp-1 italic"
                                                        title="{{ $history->evaluation }}">
                                                        "{{ \Illuminate\Support\Str::limit($history->evaluation, 40) }}"
                                                    </span>
                                                @else
                                                    <span class="text-[10px] text-gray-300 italic">
                                                        No notes
                                                    </span>
                                                @endif
                                            </td>
                                            ...
                                            <td class="px-6 py-4 text-right">
                                                <form action="{{ route('coach.attendance.destroy') }}"
                                                    method="POST"
...
function filterHistoryMonth(value) {
    document.querySelectorAll('.month-section').forEach(section => {
        section.classList.toggle(
            'active',
            value === 'all' || section.id === 'history-' + value
        );
    });
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (select) {
        const now = new Date();
        const month = now.toLocaleString('id-ID', {month: 'long'}).toLowerCase();
        const currentSlug = month + '-' + now.getFullYear();
        ...
        if (found) {
            filterHistoryMonth(currentSlug);
    ...
    document.querySelectorAll('.athlete-checkbox').forEach(checkbox => {
        updateRowStyle(checkbox.closest('tr'), checkbox.checked);
...
if (searchAthlete) {
    searchAthlete.addEventListener('keyup', function() {
